...
const Preview = () => {
  ...
  const parsedMarkdownRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    if (parsedMarkdownRef.current) {
      let html = markdown;
      rules.forEach(([rule, template]) => {
        html = html.replace(rule, template as string);
      });
      parsedMarkdownRef.current.innerHTML = html;
    }
  }, [markdown]);
  return <div ref={parsedMarkdownRef}></div>;
};
